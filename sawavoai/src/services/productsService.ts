...
export const productsService = {
  ...
  async getProductBySlug(slug: string): Promise<Product | null> {
    try {
      // First try the slug endpoint if your backend supports it
      const response = await fetch(`${API_CONFIG.API_BASE_URL}/products/slug/${slug}`);
      ...
      // Fallback: search for product by slug in the general products endpoint
      const searchResponse = await fetch(`${API_CONFIG.API_BASE_URL}/products?search=${encodeURIComponent(slug)}`);
      ...
      if (searchData.success && searchData.data && searchData.data.products) {
