 ...
 */
export async function createSafeExcerpt(content: string, maxLength: number = 160): Promise<string> {
  if (!content) return '';
...
export async function validateBlogContent(content: string, locale: string = 'en'): Promise<ContentValidationResult> {
  const errors: string[] = [];
