...
export class ProductsController {
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieves a single product by its slug for SEO-friendly URLs'
  })
  @ApiParam({
    name: 'slug',
    description: 'Product slug',
    example: 'versace-eros-pour-femme-edp-100-ml',
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Product retrieved successfully',
  ...
  async getProductBySlug(@Param('slug') slug: string): Promise<{
    success: boolean;
    data: any;
    message: string;
    timestamp: string;
